...
## 五、结论
- 通过对语音信号的调制与传输，我们成功地实现了对语音信号的幅度调制、传输和解调，并且通过设计合适的滤波器有效地去除了噪声，恢复了原始信号。
- ...【怎么没人写:angry:】
- 在滤波部分，我们尝试了多种滤波器，最终选择效果最佳的维纳滤波器，以维纳滤波器的原理设计了一个简单的滤波函数，在尝试中发现发现当噪声功率设置为7.5时，滤波效果较不传入噪声功率效果更佳
## 六、收获与感想
- ...【怎么没人写:angry:】
- 了解了多种滤波器的原理和实现方法，成功实现了简单维纳滤波器的设计与应用，只是实现的较为简单，实际应用中需要考虑更加复杂的噪声情况。由于噪声总是很难记录的，所以重点应该优化没有噪声信息传入时的估计噪声功率部分
## 七、创新点
- ...【怎么没人写:angry:】
- 在滤波部分，舍弃了常规的巴特沃斯滤波器，尝试了维纳滤波器，发现其效果更佳
